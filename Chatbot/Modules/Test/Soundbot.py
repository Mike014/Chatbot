...
from Sound_Synthesis import generate_chord, generate_melody_
...
class Soundbot:
    def __init__(self):
        self.intents = {
            "generate_sine": compile_and_search(
                r"generate sine with frequencies (\d+(?:,\d+)*) and amplitude (\d+) and duration (\d+)"
            ),
            ...
            "generate_melody": compile_and_search(
                r"generate a melody from (\w+) in (\w+) direction"
            ),
        }
    ...
    def generate_sine_intent(self, frequencies, amplitude, duration_sine):
        ...
        sine_wave = sum(
            genSine(amplitude, f, 0, 44100, duration_sine) for f in frequencies
        )
    ...
    def generate_melody_intent(self, note, direction):
        ...
        if note in notes:
            ...
            for _ in range(len(melody_notes)):
                ...
                melody_notes.remove(
                    note
                )  # remove the note from the list to avoid repeating it
                amplitude = notes[note]["amplitude"]
                frequency = notes[note]["frequency"]
                print(f"Generating {note} with frequency {frequency} Hz")
                melody_wave = np.append(
                    melody_wave, generate_melody_(amplitude, frequency, fs, t)
                )
        ...
        for intent, pattern in self.intents.items():
            ...
            if found_match:
                if intent == "generate_sine":
                    ...
                    return self.generate_sine_intent(
                        frequencies, amplitude, duration_sine
                    )
