...
class Chunker:
    def __init__(self, text, train_text=None):
        if not isinstance(text, str) or not isinstance(train_text, str):
            raise ValueError(
                "Expected a string for text and train_text, but received: %s and %s"
                % (type(text), type(train_text))
            )
    ...
    def chunk(self, chunk_gram):
        try:
            ...
            return [
                chunk_parser.parse(tagged_sentence) for tagged_sentence in pos_tagged
            ]
